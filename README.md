...
1. **테마 선택**: 10개 테마 중 학습하고 싶은 분야 선택
2. **학습 모드 선택**: 객관식 4지선다 또는 타이핑 모드 중 선택
3. **🌙 테마 변경**: 우상단 버튼으로 라이트/다크 모드 전환
4. **단어 학습**: 영어 단어를 보고 올바른 한국어 뜻 선택 또는 직접 입력
5. **🔊 발음 듣기**: 스피커 버튼을 클릭하여 원어민 발음 확인
6. **⚡ 자동 진행**: 토글을 켜면 정답 후 2초 자동 다음 문제
7. **✍️ 타이핑 모드**: 토글을 켜면 직접 입력으로 학습 (더 어려움)
8. **결과 확인**: 테마 완료 시 점수와 정답률 확인
9. **🔄 복습하기**: 틀린 문제가 있는 테마에서 "복습" 버튼 클릭
10. **📊 통계 확인**: "학습 통계" 버튼으로 상세한 학습 현황 확인
11. **🤝 점수 공유**: 완료 후 친구들과 인스타그램 스타일 점수 카드 공유
12. **🎯 친구 도전**: 도전장 생성 후 친구에게 보내고 점수 경쟁하기
13. **테마 전환**: 다른 테마 도전 또는 같은 테마 재시도
